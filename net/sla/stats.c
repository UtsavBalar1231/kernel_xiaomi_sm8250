...
/* about stats */
#define STATS_BUF_SIZE                                                         \
	(sizeof(struct sla_interface_stats)) //16*(1+ENTRY_TOTAL) = (20*1024)

#define MMAP_FILE_SUFFIX "_mmap"
#define TOTAL_STATS_FILE_SUFFIX "_stats"

static struct sla_config config_data;
static u64 pkt_alloc_num;
static u64 pkt_free_num;

LIST_HEAD(sla_iface_list);

/* about port hash list */
#define PORT_HASH_BITS 8
#define PORT_HASH_SIZE BIT(PORT_HASH_BITS)
...
static int fops_u64_open(struct inode *inode, struct file *filp)
{
	...
	return simple_attr_open(inode, filp, procfs_u64_get, procfs_u64_set,
				"%llu\n");
}
...
static struct proc_dir_entry *
	config_file; //Used for SLA config: enable, prots, reset_traffic and so on
...
static void pkt_hlist_print(void)
{
	...
	for (h = 0; h < PORT_HASH_SIZE; h++) {
		hlist_for_each_entry_rcu(pkt, &pkt_list[h], hlist)
			pr_info("%s() pkt_list[%d] local_port=%d\n", __func__, h,
				pkt->local_port);
	}
}
...
static bool sla_pkt_valid(struct sk_buff *skb, u8 link_type)
{
	...
	for (i = 0; (i < PORTS_MAX) && (config_data.ports[i] != 0); i++)
		if (ext_port == config_data.ports[i])
			break;

	if ((i == PORTS_MAX) ||
			ext_port != config_data.ports[i])
		return false;

	len = ntohs(ip_hdr(skb)->tot_len) -
	      (ip_hdr(skb)->ihl * 4 + tcp_hdrlen(skb));
	if (len == 0) {
		tcph = tcp_hdr(skb);
...
static bool sla_pkt_consolidate(struct sla_interface_stats *stats)
{
	...
	if ((stats->entries[prev].timestamp + config_data.limit_time) <
	    entry->timestamp)
		return false;
...
static unsigned int sla_ipv4_in(void *priv, struct sk_buff *skb,
...
static unsigned int sla_ipv4_out(void *priv, struct sk_buff *skb,
				 const struct nf_hook_state *state)
{
	struct sla_interface_stats *p;
	struct sla_stats_entry *entry;
			  ...
			  size_t count, loff_t *ppos)
{
	...
	return simple_read_from_buffer(user_buf, count, ppos, stats,
				       STATS_BUF_SIZE);
}
...
static int sla_interface_start(const char *interface_name)
{
	...
	if (!interface_name) {
		pr_err("### %s Cannot create iface struct when iface name is NULL\n",
		       __func__);
		return -EPERM;
	}
	...
	if (sla_find_interface(interface_name) != NULL) {
		STATS_UNLOCK_IRQ(&stats_spin_lock, flags);
		pr_err("### %s Interface %s already being tracked in SLA ",
	...
	new_interface =
		(struct sla_interface *)kmalloc(sizeof(struct sla_interface),
								GFP_ATOMIC);

	if (!new_interface) {
		pr_err("### %s kmalloc Failed for %s", __func__,
		       interface_name);
		return -EPERM;
	}

	if (strlcpy(new_interface->interface_name, interface_name,
		    sizeof(new_interface->interface_name)) > IF_NAME_LEN) {
		// the interface_name passed to the function was longer than the MAX interface length.
		pr_err("### %s invalid interface name %s", __func__,
		       interface_name);
		kfree(new_interface);
		return -EPERM;
	}

	pr_err("### %s copy the iface name into struct %s\n",
	       __func__, interface_name);

	strlcpy(mmap_file_name, new_interface->interface_name,
		sizeof(mmap_file_name));
	strlcat(mmap_file_name, MMAP_FILE_SUFFIX, sizeof(mmap_file_name));
	strlcpy(total_stats_file_name, new_interface->interface_name,
		sizeof(total_stats_file_name));
	strlcat(total_stats_file_name, TOTAL_STATS_FILE_SUFFIX,
		sizeof(total_stats_file_name));

	new_interface->mmap_struct =
		(struct sla_interface_stats *)kmalloc(STATS_BUF_SIZE, GFP_ATOMIC);

	if (!new_interface->mmap_struct) {
		pr_err("### %s kmalloc wlan_stats Fail\n", __func__);
		kfree(new_interface);
		return -EPERM;
	}
	...
	for (offset = 0; offset < STATS_BUF_SIZE; offset += PAGE_SIZE)
		SetPageReserved(virt_to_page(((u8 *)new_interface->mmap_struct)
				+ offset)); //Set the segment memory to reserved

	new_interface->mmap_file =
		proc_create_data(mmap_file_name, 0644, sla_dir, &stats_fops,
				 new_interface->mmap_struct);

	if (!new_interface->mmap_file) {
		pr_err("#### %s Fail to create file: mmap_file for interface= %s\n",
		       __func__, interface_name);
	...
	new_interface->total_stats_file =
		proc_create_data(total_stats_file_name, 0644, sla_dir,
				 &fops_u64, &new_interface->total_trfc);

	if (!new_interface->total_stats_file) {
		pr_err("#### %s Fail to create total_stats_fils for interface = %s\n",
		       __func__, interface_name);
...
static void sla_stats_disable(void)
{
	...
	list_for_each_entry_safe(iface, next, &sla_iface_list,
				 interface_list_hook) {
		pr_err("### %s trying to disable %s\n", __func__,
		       iface->interface_name);
		iface->total_trfc = 0;
		if (iface->mmap_struct) {
			memset(iface->mmap_struct, 0, STATS_BUF_SIZE);

			for (offset = 0; offset < STATS_BUF_SIZE;
			     offset += PAGE_SIZE)
				ClearPageReserved(virt_to_page(((u8 *)iface->mmap_struct)
							+ offset));
		...
		pr_err("### %s removing form list %s\n", __func__,
		       iface->interface_name);

		list_del(&iface->interface_list_hook);
		pr_err("### %s removed form list %s %p\n", __func__,
...
static int config_file_show(struct seq_file *s, void *data)
{
	...
	if (config->ports[0] != 0) {
		...
		for (i = 1; (i < PORTS_MAX) && (config->ports[i] != 0); i++)
...
static ssize_t config_file_write(struct file *filp, const char __user *buffer,
				 size_t count, loff_t *ppos)
{
	...
	if (strncmp(cmd, CMD_IFACE_ADD, len) == 0) {
		pr_err("#### %s CMD_IFACE_ADD=[%s]\n", __func__, cmd + len);
		buf = cmd + len;
		if (strlen(buf) < IF_NAME_LEN)
			sla_interface_start((const char *)buf);
		else
			pr_err("#### %s ifname=[%s] is invaild\n",
			       __func__, buf);
	...
	if (strncmp(cmd, CMD_ENABLE, len) == 0) {
		pr_err("#### %s enable=%c\n", __func__, cmd[len]);
	...
	if (strncmp(cmd, CMD_RATE_ON, len) == 0) {
		pr_err("#### %s rate_on=%c\n", __func__, cmd[len]);
		if ('1' == cmd[len] && 0 == cmd[len + 1])
			config_data.rate_on = 1;
		else if ('0' == cmd[len] && 0 == cmd[len + 1])
	...
	if (strncmp(cmd, CMD_PORTS, len) == 0) {
		pr_err("#### %s ports=[%s]\n", __func__, cmd + len);
		buf = cmd + len;
		i = 0;
	...
	if (strncmp(cmd, CMD_RESET_TRAFFIC, len) == 0) {
		if ('1' == cmd[len] && 0 == cmd[len + 1]) {
			struct sla_interface *iface = NULL;
			unsigned long flags;

			STATS_LOCK_IRQ(&stats_spin_lock, flags);
			list_for_each_entry(iface, &sla_iface_list,
					    interface_list_hook) {
				iface->total_trfc = 0;
			}
			STATS_UNLOCK_IRQ(&stats_spin_lock, flags);
			pr_err("#### %s reset_traffic=%c\n", __func__,
			       cmd[len]);
		}
	...
	if (strncmp(cmd, CMD_HASH_PRINT, len) == 0) {
		pr_err("#### %s hash print %llu, %llu\n", __func__,
		       pkt_alloc_num, pkt_free_num);
		pkt_hlist_print();
	...
	if (strncmp(cmd, CMD_HASH_CLEAR, len) == 0) {
		pr_err("#### %s hash clear\n", __func__);
		pkt_hlist_deinit();
	...
	if (strncmp(cmd, CMD_MAX_SIZE, len) == 0) {
		num = simple_strtoul(&cmd[len], NULL, 10);
		if (num < MAX_SIZE_DEFAULT) {
			pr_err("#### %s max_size=%lu is invaild, need greater than %d\n",
	...
	if (strncmp(cmd, CMD_LIMIT_TIME, len) == 0) {
		num = simple_strtoul(&cmd[len], NULL, 10);
		if (num < LIMIT_TIME_DEFAULT) {
			pr_err("#### %s limit_time=%lu is invaild, need longer than %d\n",
			       __func__, num, LIMIT_TIME_DEFAULT);
		} else {
			config_data.limit_time = num & 0xFFFF;
			pr_err("#### %s limit_time=%d\n", __func__,
			       config_data.limit_time);
		}
...
static int __init sla_static_collector_init(void)
{
	...
	pr_err("#### %s in version V 3.0\n", __func__);

	err = nf_register_net_hooks(&init_net, sla_ipv4_ops,
				    ARRAY_SIZE(sla_ipv4_ops));
	if (err < 0) {
		pr_err("%s: can't register hooks.\n", __func__);
		return err;
	}
	...
	if (!sla_dir) {
		pr_err("#### %s Fail to create sla dir\n", __func__);
		return -EPERM;
	}
	...
	config_file = proc_create_data("config", 0644, sla_dir, &config_fops,
				       &config_data);

	if (!config_file) {
		pr_err("#### %s Fail to create file: config_file\n",
		       __func__);
		return -EPERM;
	}
...
{
	pr_err("#### %s in\n", __func__);
	nf_unregister_net_hooks(&init_net, sla_ipv4_ops,
				ARRAY_SIZE(sla_ipv4_ops));
