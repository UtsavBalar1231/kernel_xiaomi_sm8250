...
#define GET_NAME_LEN(max, str) \
	((max) > (strlen(str) + 1) ? (strlen(str) + 1) : (max))

TRACE_EVENT(dma_buf_export,
	TP_PROTO(const struct dma_buf_export_info *exp_info,
		 unsigned int pid),
...
TRACE_EVENT(dma_buf_map_attachment,
	...
	TP_fast_assign(
		memcpy(__entry->dmabuf_name, attach->dmabuf->name,
			GET_NAME_LEN(DMABUF_NAME_MAX_LEN,
				     attach->dmabuf->name));
...
TRACE_EVENT(dma_buf_mmap_internal,
	...
	TP_PROTO(struct file *file,
		 struct vm_area_struct *vma, unsigned int pid),
