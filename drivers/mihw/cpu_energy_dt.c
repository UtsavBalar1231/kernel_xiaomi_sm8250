...
static DEFINE_PER_CPU(struct ea_state*, cpu_eas) = NULL;
static DEFINE_PER_CPU(unsigned long, nr_cap_stats) = 0;
static DEFINE_PER_CPU(struct id_state*, cpu_ids) = NULL;
static DEFINE_PER_CPU(unsigned long, nr_ids_stats) = 0;
			       ...
			       void *data)
{
	...
	for_each_cpu(cpu, policy->cpus) {
		if (per_cpu(nr_cap_stats, cpu)
			|| per_cpu(cpu_eas, cpu)
			|| per_cpu(nr_ids_stats, cpu)
			|| per_cpu(cpu_ids, cpu)) {
			ret = -EEXIST;
			goto unlock;
		}
	...
	for (i = 0, freq = 0; i < cap_nstats; i++, freq++) {
		ret = of_dev_pm_opp_get_cpu_power(&eas[i].power,
				&freq, cpu);
		if (unlikely(ret)) {
			kfree(eas);
...
void cpu_ids_dt(int cpu, unsigned int idle_index, unsigned int *power)
{
	if (power)
		*power = 0;
/*
	if (unlikely(idle_index >= per_cpu(nr_ids_stats, cpu)))
		idle_index = per_cpu(nr_ids_stats, cpu) - 1;
	if (power)
		*power = per_cpu(cpu_ids, cpu)[idle_index].power;
*/
}
