...
#ifdef CONFIG_MIUI_BUILD
#include <linux/trace_clock.h>
#endif

#ifdef CONFIG_MI_SCHED
extern void mi_binder_restore_vip_hook(struct binder_transaction *in_reply_to,
				       struct task_struct *task);
extern void mi_binder_set_vip_hook(struct binder_transaction *t,
				   struct task_struct *task);
#endif
...
static int binder_open(struct inode *nodp, struct file *filp)
{
...
#ifdef CONFIG_MIUI_BUILD
		binder_binderfs_dir_entry_proc_transaction =
			info->proc_transaction_log_dir;
#endif
	} else {
	...
	if (binder_debugfs_dir_entry_proc_transaction) {
		...
		snprintf(strbuf, sizeof(strbuf), "%u", proc->pid);
		proc->debugfs_transaction_entry = debugfs_create_file(
			strbuf, 0444, binder_debugfs_dir_entry_proc_transaction,
			(void *)(unsigned long)proc->pid,
	...
	if (binder_binderfs_dir_entry_proc_transaction) {
		...
		snprintf(strbuf, sizeof(strbuf), "%u", proc->pid);
		binderfs_transaction_entry = binderfs_create_file(
			binder_binderfs_dir_entry_proc_transaction, strbuf,
			&proc_transaction_fops,
			(void *)(unsigned long)proc->pid);
		if (!IS_ERR(binderfs_transaction_entry)) {
			proc->binderfs_transaction_entry =
				binderfs_transaction_entry;
		} else {
			int error;
...
#ifdef CONFIG_MIUI_BUILD
static void print_binder_proc_transaction_ilocked(struct seq_file *m,
						  struct binder_proc *proc,
						  const char *prefix,
						  struct binder_transaction *t,
						  u64 timesRecord)
...
static void print_binder_proc_work_transaction_ilocked(struct seq_file *m,
						       struct binder_proc *proc,
						       const char *prefix,
						       struct binder_work *w,
						       u64 timesRecord)
{
	...
	case BINDER_WORK_TRANSACTION:
		t = container_of(w, struct binder_transaction, work);
		print_binder_proc_transaction_ilocked(m, proc, prefix, t,
						      timesRecord);
		break;
	default:
		break;
					       ...
					       u64 timesRecord)
{
		...
		while (t) {
			if (t->from == thread) {
				print_binder_proc_transaction_ilocked(
					m, proc, "outgoing transaction", t,
					timesRecord);
				t = t->from_parent;
			} else if (t->to_thread == thread) {
				print_binder_proc_transaction_ilocked(
					m, proc, "incoming transaction", t,
					timesRecord);
				t = t->to_parent;
			} else {
				t = NULL;
	...
	for (n = rb_first(&proc->nodes); n != NULL; n = rb_next(n)) {
		...
		binder_node_inner_lock(node);
		list_for_each_entry (w, &node->async_todo, entry)
			print_binder_proc_work_transaction_ilocked(
				m, proc, "pending async transaction", w,
				timesRecord);
		binder_node_inner_unlock(node);
		last_node = node;
		binder_inner_proc_lock(proc);
	...
	list_for_each_entry (w, &proc->todo, entry)
		print_binder_proc_work_transaction_ilocked(
			m, proc, "pending transaction", w, timesRecord);
	binder_inner_proc_unlock(proc);
...
static void print_binder_proc_info(struct seq_file *m, struct binder_proc *proc)
{
	...
	seq_printf(m,
		   "requested_threads(%d+%d/%d)\t ready_threads(%d)\t"
		   "free_space(%zd)\tfree_async_space(%zd)\n",
		   proc->requested_threads, proc->requested_threads_started,
		   proc->max_threads, ready_threads, free_space,
		   free_async_space);
	binder_inner_proc_unlock(proc);
}
...
#ifdef CONFIG_MIUI_BUILD
		binder_debugfs_dir_entry_proc_transaction = debugfs_create_dir(
			"proc_transaction", binder_debugfs_dir_entry_root);
#endif
	}
