...
static void gpio_debounce_work(struct work_struct *work)
{
	...
	for (i = 0; i < ant_data->count; i++) {
		if (ant_data->data[i].irq == ant_data->current_irq) {
			int gpio_status =
				gpio_get_value(ant_data->data[i].gpio_num);

			if (gpio_status == ant_data->data[i].gpio_status) {
				snprintf(status_env, MAX_MSG_LENGTH,
					 "STATUS=%d", gpio_status);
				dev_info(dev,
					 "Update testing mode status: %d\n",
					 gpio_status);
...
static int md_testing_mode_probe(struct platform_device *pdev)
{
	...
	ant_data->data = devm_kcalloc(dev, ant_data->count,
				      sizeof(struct gpio_data), GFP_KERNEL);
	if (!ant_data->data)
		return -ENOMEM;

	if (of_property_read_u32(np, "debounce-time",
				 &ant_data->debounce_time)) {
	...
	for (i = 0; i < ant_data->count; i++) {
		...
		pr_info("GPIO [%d] status: %d\n", i,
			ant_data->data[i].gpio_status);

		ret = devm_request_threaded_irq(
			dev, ant_data->data[i].irq, NULL,
			md_testing_threaded_irq_handler,
			IRQF_ONESHOT | IRQF_TRIGGER_RISING |
				IRQF_TRIGGER_FALLING,
			"md_testing_mode", ant_data);
		if (ret < 0) {
