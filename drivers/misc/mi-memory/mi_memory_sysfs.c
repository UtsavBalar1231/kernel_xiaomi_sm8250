...
static int __init mi_memory_sysfs_init(void)
{
	...
	mi_memory->mem_dev = device_create(mi_memory->mem_class, NULL,
	MKDEV(mi_memory->major, MEMORYDEV_MINOR), NULL, MI_MEMORY_DEVICE);
...
memdev_unreg:
	device_destroy(mi_memory->mem_class,
		MKDEV(mi_memory->major, MEMORYDEV_MINOR));
chrdev_unreg:
