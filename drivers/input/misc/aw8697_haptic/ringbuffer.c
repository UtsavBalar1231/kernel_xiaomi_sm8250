...
struct rb {
	...
	int32_t aval_size; // avalibale to write size.
...
int write_rb(const char *data, int32_t size)
{
	...
	pr_debug("%s: write index %d, read index %d, free size %d",
		__func__, tail, head, grb->aval_size);
	...
	while ((grb->aval_size < size) && (!atomic_read(&grb->exit))) {
		...
		ret = wait_event_interruptible(grb->wait_q,
				atomic_read(&grb->buf_condition) == 1);
		if (ret == -ERESTARTSYS) {
...
int read_rb(char *data, int32_t size)
{
	...
	pr_debug("%s: data:%p, size %d", __func__, data, (int)size);
	...
	pr_debug("write index %d, read index %d, filled size %d",
		tail, head, filled_size);

	read_bytes = MIN(size, filled_size);
	if (size > filled_size)
		pr_debug("buffer underrun , req size %d, filled size %d",
			size, filled_size);
	...
	pr_debug("%s: after read: %d write index %d, read index %d, aval_size %d",
