...
static int aw8697_haptic_rtp_init(struct aw8697 *aw8697)
{
	...
	pm_qos_add_request(&pm_qos_req, PM_QOS_CPU_DMA_LATENCY, CPU_LATENCY_QOC_VALUE);
	aw8697->rtp_cnt = 0;
...
static void set_gain(struct work_struct * work)
{
	...
	if ((aw8697->activate_mode == AW8697_HAPTIC_ACTIVATE_RAM_LOOP_MODE) &&
		...
		pr_debug("%s: enable vbat comp, level = %x comp_level = %x", __func__,
				     ...
				     const char *buf, size_t count)
{
	...
	offset = 0;
	pr_debug(" write szie %zu, period size %lu", count, period_size);
