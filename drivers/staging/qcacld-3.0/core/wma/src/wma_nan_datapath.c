...
void wma_add_sta_ndi_mode(tp_wma_handle wma, tpAddStaParams add_sta)
{
	...
	if (cdp_find_peer_exist_on_vdev(soc, add_sta->smesessionId,
					add_sta->staMac)) {
		WMA_LOGE(FL("NDI peer already exists, peer_addr "QDF_MAC_ADDR_FMT),
	...
	if (cdp_find_peer_exist(soc, pdev_id, add_sta->staMac)) {
		WMA_LOGE(FL("peer exists on other vdev with peer_addr "QDF_MAC_ADDR_FMT),
			 QDF_MAC_ADDR_REF(add_sta->staMac));
		add_sta->status = QDF_STATUS_E_EXISTS;
		goto send_rsp;
	}
	...
	if (status != QDF_STATUS_SUCCESS) {
		WMA_LOGE(FL("Failed to create peer for "QDF_MAC_ADDR_FMT),
			 QDF_MAC_ADDR_REF(add_sta->staMac));
	...
	if (!cdp_find_peer_exist_on_vdev(soc, add_sta->smesessionId,
					 add_sta->staMac)) {
		WMA_LOGE(FL("Failed to find peer handle using peer mac "QDF_MAC_ADDR_FMT),
			 QDF_MAC_ADDR_REF(add_sta->staMac));
	...
	WMA_LOGD(FL("Moving peer "QDF_MAC_ADDR_FMT" to state %d"),
